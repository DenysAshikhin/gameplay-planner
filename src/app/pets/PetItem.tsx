      ...
      {filterBonuses(pet.BonusList, (bonus) => {
        ...
        return (
          <li key={i}>
            ...
            {helper.formatNumberString(result)}
          </li>
        );
      })}
    </ul>
    ...
    <ul style={{ margin: "0 0 0 0" }}>
      {filterBonuses(
        ...
        return (
          <li key={i}>
            {BonusMap[activePetBonus.ID]?.label}:{" "}
            {helper.formatNumberString(Number(activePetBonus.Power) * 100) +
              "%"}
          </li>
        );
      })}
      ...
      tooltip={
        <div className="tooltip-custom ">
          <h3 style={{ marginTop: "0" }}>
            <div>{`${name} (${promotion}*)  (${totalScore})`}</div>
            <div>
              (Level: {level}) (Rank: {rank}) ({location})
            </div>
          </h3>
          <div>
            <h4 style={{ margin: "6px 0 6px 0" }}>Active Bonuses</h4>
            {section1Bonuses}
          </div>
          <div>
            <h4 style={{ margin: "6px 0 6px 0" }}>Expedition Bonuses:</h4>
            {section2Bonuses}
          </div>
        </div>
      }
 ...
 */
export function PetItemExpeditions({
  petData,
  isSelected,
  onClick,
  data,
...
export function StaticPetItem({
  ...
  const getWarnIcon = () => {
    ...
    const showNotInCacheWarning =
      groupsCacheRunTime.length != 0 && !groupsCacheRunTime[pet.ID];
    let warning = "";
    let showBlue = false;
    if (showNotInCacheWarning && showNotInSaveWarning) {
      warning =
        "This pet is neither in an suggested expedition team nor in one active team in your save file, you should probably put it in";
    } else if (showNotInCacheWarning) {
      warning =
        "This pet is not in an suggested expedition team, you should probably put it in";
    } else {
      showBlue = true;
      warning =
        "This pet is not in an active expedition team from your save file, you should probably put it in";
    }
        ...
        tooltip={
          <div className="tooltip-custom ">
            ...
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <div>{`Rank: ${pet.Rank}`}</div>
              <div style={{ margin: "0 6px" }}>{`Level: ${pet.Level}`}</div>
              <div>{`Damage: ${helper.formatNumberString(baseDmg)}`}</div>
            </div>
            <div>
              <h4 style={{ margin: "6px 0 6px 0" }}>Active Bonuses</h4>
              {section1Bonuses}
