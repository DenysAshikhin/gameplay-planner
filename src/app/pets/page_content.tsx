      ...
      >
        ...
        >
          ...
          <MouseOverPopover
            ...
            tooltip={
              <div>
                Green: Combo available and used (counts for achievement).
                <br />
                Yellow: Combo available but unused.
                <br />
          ...
          {petCombos &&
            petCombos.map((petCombo, i) => {
              ...
              if (i === 0) {
                ...
                possibleCombosMap[petCombo.BonusID][petCombo.ID] ===
                possibleCombosMap[petCombos[i - 1].BonusID][petCombos[i - 1].ID]
              ) {
                margin = "-5px 0 0 0";
              } else {
                margin = "0 0 0 0";
                ...
                >
                  {PetIDArray.map((petId, j) => {
                    let staticPetData = petNameArray.find(
                      (staticPetDatum) => staticPetDatum.petId === petId,
                    );
                    return (
                      <div
                        key={j}
                        style={{
                          ...
                          height: "90px",
                          opacity:
                            petId in unlockedPets || petId === -99 ? `` : `0.6`,
                        ...
                        className={
                          petId in unlockedPets || petId === -99
                            ? `darkGrayBackground`
                            : ``
                        }
                      >
                        {petId !== -99 && (
                          <StaticPetItem
                            petData={{ ...staticPetData, pet: petMap[petId] }}
                            highlight={petId in unlockedPets}
                          />
                        )}
                      </div>
                    );
...
export default function Pets() {
  ...
  const petIdsInUseInSave = useMemo(
    () => petHelper.getPetIdsInExpeditionFromSaveFile(data),
    [data],
  );
  ...
  const [clientUseExpedition, setUseExpedition] = useLocalStorage(
    "useExpedition",
    false,
  );
  ...
  let preGeneratedTeams = useMemo(() => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      let airPets, groundPets, currentBonuses, selectedPetMap;
      [airPets, groundPets, currentBonuses, selectedPetMap] =
    ...
    let poopCombo: {
      satisfied: boolean;
      requiredID: { 4: boolean; 16: boolean };
      team?: any[];
    } = {
      satisfied: false,
      requiredID: {
    ...
    let newPriorityList = JSON.parse(
      JSON.stringify(statTeamMasterList.priorityList),
    );

    let newPriorityMap = JSON.parse(
    ...
    for (const [key, value] of Object.entries(newPriorityMap)) {
      ...
      //Do not reset card power or exp, also reinc and ir for lower A fallback
        ...
        key in existingStats &&
        key !== "21" &&
        key !== "22" &&
        key !== "5" &&
        key !== "6" &&
        data.AscensionCount > 29 &&
        key !== "1" &&
        data.AscensionCount > 39 &&
        key !== "3"
      ) {
        newPriorityMap[key].count = 0;
      }
  ...
  comboList.forEach((combo, index) => {
    ...
    if (partial === required) {
      if (!specialCombos[combo.BonusID]) {
        specialCombos[combo.BonusID] = {
          ...combo,
          rootName: BonusMap[combo.BonusID].rootName,
          odd: BonusMap[combo.BonusID].odd,
          name: BonusMap[combo.BonusID].label,
          count: 1,
        };
      } else {
  ...
  const positiveRankedPets = data.PetsCollection.filter((pet) => {
    ...
    petMap[pet.ID] = pet;
    const unlocked = petHelper.petUnlocked(data, pet);

    if (!useExpedition) {
      return unlocked;
    }

    const manualState = manualEnabledPets[pet.ID];
    const isValidLocked = (unlocked || manualState === 1) && manualState !== 0;
  ...
  for (const [key, value] of Object.entries(BonusMap)) {
    if (
      value.id >= bonusCutOff ||
      priorityMap[key] ||
      !bonusesWithPets[value.id] ||
      (data.AscensionCount > 29 && value.id === 1) ||
      (data.AscensionCount > 39 && value.id === 3)
    ) {
      continue;
    }
    ...
    >
      ...
      >
        ...
        <>
          ...
          {true && (
            <div
              className="teamBuilder importantText"
              style={{
                borderRadius: "6px",
                border: `2px solid rgba(255,255,255,0.7)`,
                margin: "12px 24px",
                      ...
                      <select
                        ...
                        onChange={(selected_mode) => {
                          ...
                          switch (selected_mode.target.value) {
                            ...
                            case "Stat Team":
                              setPriorityList(
                                JSON.parse(JSON.stringify(statPriorityList)),
                              );
                              setPriorityMap(
                                JSON.parse(JSON.stringify(statPriorityMap)),
                              );
                              presetPets = JSON.parse(
                                JSON.stringify(statPriorityWhitelist),
                              );
                              setStatMode(true);
                              break;
                            case "None":
                          ...
                          >
                            <div
                              ...
                              style={{
                                ...
                                display:
                                  (data.AscensionCount > 29 && item === 1) ||
                                  (data.AscensionCount > 39 && item === 3)
                                    ? "none"
                                    : "flex",
                                alignItems: "center",
                                flexDirection: "column",
              ...
              <div style={{ marginRight: "6px", marginTop: "6px" }}>
                    ...
                    <>
                      ...
                      <div style={{ padding: "12px" }}>
                        {Object.values(currentBonuses).map((e: any, index) => {
                          ...
                          return (
                            ...
                            >
                              <div style={{ width: "150px" }}>{e.label}</div>
                              <div
                                style={{
                                  display: "flex",
                                  alignItems: "center",
                              ...
                              >
                                ...
                                <div
                                  style={{ color: color, marginLeft: "6px" }}
                                >
                      ...
                      {Object.values(specialCombos).length > 0 && (
                        <>
                          ...
                          <div style={{ padding: "12px" }}>
                            {Object.values(specialCombos).map(
                              (e: any, index) => {
                                return (
                      ...
                      <div>
                        ...
                        <div style={{ display: "flex" }}>
                          {groundPets.map((e, index) => {
                            let staticPetData = petNameArray.find(
                              (staticPetDatum) => staticPetDatum.petId === e.ID,
                            );
                            return (
                              <div
                                key={index}
                ...
                >
                  ...
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <div style={{}}>Use pets from /expeditions page</div>
                    <div>
                      <input
                  ...
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <div style={{}}>Ignore Promotions</div>
                    <div>
                      <input
                        aria-label="use selected pets from expedition page"
                        type="checkbox"
                ...
                >
                  ...
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    ...
                    />
                    <button
                      disabled={currentPresetName.trim().length === 0}
                      onClick={(e) => {
                        ReactGA.event({
                          category: "pets_interaction",
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(selected_mode) => {
                        ...
                        setPriorityList(
                          customPresets[selected_mode.target.value]
                            .priorityList,
                        );
                        setPriorityMap(
                          customPresets[selected_mode.target.value].priorityMap,
                        );
                        let petWhiteListNew = {};
                        for (const [key, value2] of Object.entries(
                          customPresets[selected_mode.target.value]
                    ...
                    >
                      ...
                      {Object.keys(customPresets).map((e) => (
                ...
                >
                  ...
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    ...
                    />
                    <button
                      disabled={loadPreset.trim().length === 0}
                      onClick={(e) => {
                        try {
                          ReactGA.event({
                            category: "pets_interaction",
