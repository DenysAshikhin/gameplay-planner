...
import { zone_priority, zone_ratios, zone_data, calc_max_hp, calc_total_hp, card_priority } from './zone_lists';
    ...
    const current_cards = useMemo(() => {
        ...
        if (pinnedZones) {
            for (const [key, value] of Object.entries(pinnedZones)) {
                ...
                (value as any).CardFound.forEach((found_id) => {
        ...
        const cards = data.CardsCollection.reduce((accum, card) => {
            (card as any).num_drops = num_drops[card.ID] ? num_drops[card.ID] : 0;
