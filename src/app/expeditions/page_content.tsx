...
function ScoreSection({ data, group, totalScore, defaultRank }) {
  ...
  const displayedDamage = helper.formatNumberString(
    mathHelper.multiplyDecimal(data.petDamageBD, score * 5),
  );
  return (
    <>
      <ul>
        ...
        <li key="totalScore">{`Per Rank Damage: ${displayedDamage}`}</li>
        <li key="baseGroupScore">
          Group Base: {helper.formatNumberString(Number(baseGroupScore))}
        </li>
        <li key="damageBonus">
          Dmg Bonus:{" "}
          {helper.formatNumberString(
            Number(1 + dmgCount * petHelper.EXP_DMG_MOD),
          )}
          x
        </li>
        <li key="timeBonus">
          Time Bonus:{" "}
          {helper.formatNumberString(
            Number(1 + timeCount * petHelper.EXP_TIME_MOD),
          )}
          x
        </li>
        <li key="synergyBonus">
...
export default function Expeditions() {
  ...
  const [petWhiteListClient, setPetWhiteList] = useLocalStorage(
    "petWhiteList",
    [],
  );
  useEffect(() => {
  ...
  const [manualEnabledPetsClient, setManualEnabledPets] = useLocalStorage(
    "manualEnabledPets",
    {},
  );
  useEffect(() => {
  ...
  const [clientMaxTopStat, setMaxTopStat] = useLocalStorage(
    "maxTopStat",
    false,
  );
  ...
  const [clientIgnorePetRanks, setIgnorePetRanks] = useLocalStorage(
    "ignorePetRanksPets",
    false,
  );
  ...
  const [clientEqualisePets, setEqualisePets] = useLocalStorage(
    "clientEqualisePets",
    false,
  );
  ...
  const [enabledBonusHighlightClient, setEnabledBonusHighlight] =
    useLocalStorage("enabledBonusHighlight", {});
  useEffect(() => {
  ...
  const [showAllBonusTallyClient, setShowAllBonusTally] = useLocalStorage(
    "showAllBonusTally",
    false,
  );
  useEffect(() => {
  ...
  const [leftOverBonus1Client, setLeftOverBonus1] = useLocalStorage(
    "leftOverBonus1",
    1016,
  );
  useEffect(() => {
  ...
  const [hideLockedClient, setHideLocked] = useLocalStorage(
    "hideLocked",
    false,
  );
  useEffect(() => {
  ...
  const [activeCustomBonusesClient, setActiveCustomBonuses] = useLocalStorage(
    "activeCustomBonuses",
    [],
  );
  useEffect(() => {
  ...
  const [groupRankCriteraClient, setGroupRankCriteria] = useLocalStorage(
    "groupRankCriteria",
    1,
  );
  useEffect(() => {
  ...
  const [comboSelectorClient, setComboSelector] = useLocalStorage(
    "comboSelector",
    1,
  );
  useEffect(() => {
  ...
  const [groupsCacheClient, setGroupsCache] = useLocalStorage(
    "groupsCache",
    {},
  );
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const localPets = selectedItems
      .filter((e) => e < 9999)
      .map((petId) => selectedItemsById[petId]);
    ...
    if (groups && !recalculate) {
      ...
      groups.forEach((inner_group) => {
        inner_group.forEach((group_pet) => {
          ...
          finalPet.name = petNames[finalPet.ID]
            ? petNames[finalPet.ID].name
            : petNames[9999].name;
  ...
  useEffect(() => {
    if (dataLoaded.current === false && manualEnabledPetsLoaded) {
      ...
      uploadedData.PetDamageBonuses = mathHelper.createDecimal(
        uploadedData.PetDamageBonusesBD,
      ) as any;
      ...
      const positiveRankedPets = uploadedData.PetsCollection.filter((pet) => {
        const isValidLocked = includeLocked
          ? true
          : petHelper.petUnlocked(uploadedData, pet);
        if (isValidLocked) {
          origPets.push(pet);
        }
        //If set to 0, we manually want this pet disabled
        if (
          (isValidLocked && manualEnabledPets[pet.ID] !== 0) ||
          manualEnabledPets[pet.ID] === 1
        ) {
          tempPets.push(pet);
          return true;
        }
  ...
  const finalTokenBonus =
    (data.ExpeditionTokenBonuses *
      data.ExpeditionResourceBonuses *
      comboSelector) /
    defaultSpecialCombo;
  ...
  if (groups.length > 0)
    groups.map((group, index) => {
      ...
      damageTotal = mathHelper.addDecimal(
        damageTotal,
        mathHelper.multiplyDecimal(
          petDamageBD,
          petHelper.calculateGroupScore(group, 0).groupScore * 5,
        ),
      );
      group.forEach((pet) => {
  ...
  for (const [key, value] of Object.entries(bonusTotals)) {
    if (
      activeCustomBonuses.find((a) => a.id === Number(key)) ||
      showAllBonusTally
    )
      totalMessages.push({
  ...
  selectedPets.map((e, index) => {
    let found = e.BonusList.find(
      (inner_bonus) => inner_bonus.ID === leftOverBonus1,
    );
    if (found) {
      let tempy = { ...e };
  ...
  leftOver1Pets = leftOver1Pets.sort(
    (a, b) =>
      petHelper.calculatePetBaseDamage(b, defaultRank) -
      petHelper.calculatePetBaseDamage(a, defaultRank),
  );
    ...
    >
        ...
        >
          ...
          <div style={{ overflow: "auto" }}>
            {groups.reduce((accum, group, index) => {
              ...
              const groupTotal = petHelper.calculateGroupScore(
                group,
                defaultRank,
              );
              ...
              let tempTokenScore = petHelper.calculateBestHours(
                group,
                null,
                {
                  clover: data.SoulGoldenClover,
              ...
              let tokenScore = helper.formatNumberString(
                tempTokenScore.tokenHR / tempTokenScore.hours,
                3,
              );
              ...
              const displayedDamage = helper.formatNumberString(
                mathHelper.multiplyDecimal(
                  petDamageBD,
                  groupTotal.groupRankScore * 5,
                ),
              );
              // const trueDamage = (5 * groupTotal.groupScoreMax * Number(data?.PetDamageBonuses)).toExponential(2);
              const trueDamage = helper.formatNumberString(
                mathHelper.multiplyDecimal(
                  petDamageBD,
                  5 * groupTotal.groupScoreMax,
                ),
              );

              let tokenInfo:
                | ReturnType<typeof petHelper.calculateBestHours>
                | string = ``;
              ...
              switch (groupRankCritera) {
                case 1: //damage
                  ...
                  tokenInfo = petHelper.calculateBestHours(
                    group,
                    null,
                    {
                      clover: data.SoulGoldenClover,
              ...
              const totalScore = helper.formatNumberString(
                mathHelper.multiplyDecimal(petDamageBD, score * 5),
                3,
              );
              const groupTooltip = (
              ...
              let GroupTitle = (
                ...
                >
                  ...
                  >
                    ...
                    >
                      <div style={{ margin: "0 12px" }}>{groupLabelDamage}</div>
                    </div>
                    <div
                      style={{
                        width: "33%",
                    ...
                    >
                      <div style={{ margin: "0 12px" }}>{groupLabelToken}</div>
                    </div>
                  </div>
                </div>
              );
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            ...
                            onClick={(e) => {
                              setPetWhiteList((curr) => {
                                ...
                                let pet_inner = temp.find(
                                  (sample_pet) => sample_pet.id === petData.ID,
                                );
                                if (!pet_inner) {
                                  temp.push({
                          ...
                          >
                            ...
                            >
                              <Image fill src={pinIcon} alt="push pin" />
                            </div>
                            {/* <img
                                                        alt='push pin'
                                                        style={{ width: '20px' }}
                          ...
                          >
                            ...
                            >
                              <Image src={trashIcon} alt="trash can" fill />
                            </div>
                            {/* <img alt='trash can'
                                                        style={{ width: '20px' }}
                                                        src={trashIcon} /> */}
      ...
      >
        ...
        >
          ...
          <div
            style={{ height: "30px", alignSelf: "center", marginLeft: "12px" }}
          >
            <MouseOverPopover
              opacity={1}
              tooltip={
                <div>
                  ...
                  <Image
                    alt="screenshot of game text explaining how pet ranks work"
                    src={rankExplain}
                    style={{ height: "50px" }}
                  />
                  <div style={{ display: "flex", alignItems: "center" }}>
                    ...
                    <div style={{ marginLeft: "12px", maxWidth: "724px" }}>
                      Whitelist (make sure they are on an expedition team) any
                      important pets (such as those that have item rating,
                      reincarnation exp, residue) or ones that you have equipped
                      often. The expedition ranks will make those pets level
                      faster and give bigger bonuses when actively equipped{" "}
                    </div>
                  </div>
                </div>
              }
            ...
            <div style={{ marginRight: "auto" }}>
              ...
              <div style={{ display: "flex", alignItems: "center" }}>
                <MouseOverPopover
                  tooltip={
                    <div>
                      Expedition reward from active pets special combo (0, 10%,
                      20%)
                    </div>
                  }
                >
                  <div style={{ marginRight: "12px" }}>Exp. Reward Combo</div>
                </MouseOverPopover>
                <select
                  className="importantText"
                  style={{
            ...
            >
              ...
              >
                {totalMessages.map((e, index) => {
                  ...
                  let secondDmgBias: any =
                    index + 1 < totalMessages.length
                      ? Number(totalMessages[index + 1].bonus)
                      : null;

                  let firstFailMsg = failedFilters[e.bonus];
                  let secondFailMsg = secondDmgBias
                    ? failedFilters[totalMessages[index + 1].bonus]
                    : null;
                            ...
                            onChange={(num) => {
                              try {
                                ...
                                setActiveCustomBonuses((bonuses) => {
                                  ...
                                  let bonus = newBonuses.find(
                                    (a) => a.id === active_bon.id,
                                  );
                                  bonus.relThresh = x;
                                  return newBonuses;
                                });
                    ...
                    >
                      ...
                      >
                        ...
                        >
                            ...
                            <div style={{ display: "flex" }}>
                              ...
                              <div
                                style={{
                                  width: "24px",
                                  background:
                                    helper.bonusColorMap[e.bonus].color,
                                }}
                              />
                            </div>
                          </div>
                                ...
                                tooltip={
                                  <div>
                                    ...
                                    <div>
                                      ...
                                      stronger to considered, lower means
                                      smaller threshold to slot them in
                                    </div>
                                    <div>
                                      This is based on the best team at each
                                      step without these pets, vs with it.
                                    </div>
                                  </div>
                                }
                              >
                          ...
                          {!!firstFailMsg && (
                            <div style={{ color: "red" }}>{firstFailMsg}</div>
                          )}
                        </div>
                      </div>
                      {index + 1 < totalMessages.length && (
                              ...
                              <div style={{ display: "flex" }}>
                                ...
                                <input
                                  ...
                                  onChange={(e_inner) => {
                                    setEnabledBonusHighlight({
                                      ...enabledBonusHighlight,
                                      [totalMessages[index + 1].bonus]: e_inner
                                        .target.checked
                                        ? 1
                                        : 0,
                                    });
                  ...
                  <div style={{ display: "flex", justifyContent: "center" }}>
                    <div style={{ marginTop: "5px" }}>
                      <button
                        onClick={(e) => {
                          setPetWhiteList((curr) => {
                            let temp = [...curr].filter(
                              (elem) => elem.placement === PLACEMENT_BLACKLIST,
                            );
                            return temp;
                          });
                        }}
                ...
                >
                  <SearchBox
                    ...
                    onSelect={(e) => {
                      setPetWhiteList((curr) => {
                        let temp = [...curr];
                        let petObj = data.PetsCollection.find(
                          (search_pet) => search_pet.ID === e.id,
                        );
                        temp.push({
                          ...e,
                  ...
                  >
                    ...
                    >
                      <div style={{ marginRight: "6px" }}>Team Presets</div>
                      <select
                        className="importantText"
                        style={{
                          maxWidth: "144px",
                        ...
                        onChange={(e) => {
                          let selectedTeam =
                            data.PetsLoadout[Number(e.target.value)];
                            ...
                            for (let x = 0; x < selectedTeam.IDs.length; x++) {
                              ...
                              if (selected > 0) {
                                let base = {
                                  ...
                                  placement: PLACEMENT_AUTO,
                                  parameters: { team: 0, damageBias: 17 },
                                  pet: data.PetsCollection.find(
                                    (pet_search) => pet_search.ID === selected,
                                  ),
                                };
                                if (
                                  !temp.find(
                                    (inner_find) => inner_find.id === base.id,
                                  )
                                ) {
                                  temp.push(base);
                                }
                      ...
                      >
                        {[
                          <option value="" key={"initial one"}>
                            Select Team
                          </option>,
                          ...data.PetsLoadout.map((cur, index) => {
                    ...
                    >
                      ...
                      <select
                        ...
                        onChange={(selected_mode) => {
                          ...
                          switch (selected_mode.target.value) {
                            case "Main Team":
                              priorityList = JSON.parse(
                                JSON.stringify(
                                  mainTeamSuggestions[ascensionKey]
                                    .priorityList,
                                ),
                              );
                              priorityMap = JSON.parse(
                                JSON.stringify(
                                  mainTeamSuggestions[ascensionKey].priorityMap,
                                ),
                              );
                            ...
                            case "Gear Team":
                              ...
                              priorityMap = JSON.parse(
                                JSON.stringify(
                                  gearTeamSuggestions[ascensionKey].priorityMap,
                                ),
                              );
                            ...
                            case "Portal Starter Team":
                              ...
                              presetPets = portalStarterTeamSuggestions[
                                ascensionKey
                              ].petWhiteList
                                ? JSON.parse(
                                    JSON.stringify(
                            ...
                            case "Seed Team":
                              ...
                              priorityMap = JSON.parse(
                                JSON.stringify(
                                  seedTeamSuggestions[ascensionKey].priorityMap,
                                ),
                              );
                            ...
                            case "Subclass Exp Team":
                              ...
                              presetPets = subclassExpTeamSuggestions[
                                ascensionKey
                              ].petWhiteList
                                ? JSON.parse(
                                    JSON.stringify(
                            ...
                            case "Stat Team":
                              priorityList = JSON.parse(
                                JSON.stringify(statTeamMasterList.priorityList),
                              );
                              priorityMap = JSON.parse(
                                JSON.stringify(statTeamMasterList.priorityMap),
                              );
                              presetPets = statTeamWhitelistByAscension[
                                ascensionKey
                              ]
                                ? JSON.parse(
                                    JSON.stringify(
                      ...
                      >
                        ...
                        {renownTeamVisible && (
                          <option value="Renown Team">Renown Team</option>
                        )}
                        {portalStarterTeamVisible && (
                          <option value="Portal Starter Team">
                            Portal Starter Team
                          </option>
                        )}
                        {seedTeamVisible && (
                          <option value="Seed Team">Seed Team</option>
                        )}
                        {subclassExpTeamVisible && (
                          <option value="Subclass Exp Team">
                            Subclass Exp Team
                          </option>
                        )}
                        {recommendedSelected && (
                  ...
                  <MouseOverPopover
                    tooltip={
                      <div style={{ padding: "6px" }}>
                        ...
                        <div>Blacklist: Omits this pet from any group</div>
                        <div>
                  ...
                  <MouseOverPopover
                    tooltip={
                      <div style={{ padding: "6px" }}>
                        <div>
                          <div>
                            In Placement=Group, determines which group the pet
                            is placed in
                          </div>
                          <div>
                            In Placement=Auto, tried to find the optimal
                    ...
                    if (pet.placement !== PLACEMENT_BLACKLIST) {
                      ...
                      if (group_index > -1) {
                        ...
                        if (group_index !== groups.length - 1) {
                          ...
                          const maxChecks = 2;
                          let originalGroupScore =
                            petHelper.calculateGroupScore(
                              group,
                              defaultRank,
                            ).groupScore;
                          let tempGroup = [];
                          ...
                          for (let i = 0; i < maxChecks; i++) {
                            ...
                            for (
                              ...
                              if (temp_pet.Type === pet.pet.Type) {
                                if (!(temp_pet.ID in triedPets) && !foundNew) {
                                  ...
                                  foundNew = true;
                                  tempGroup = [...group];
                                  let ind = tempGroup.findIndex(
                                    (temp_repl) => temp_repl.ID === pet.pet.ID,
                                  );
                            ...
                            let newGroupScore = petHelper.calculateGroupScore(
                              tempGroup,
                              defaultRank,
                            ).groupScore;
                            ...
                            for (
                              ...
                              if (temp_pet.Type === pet.pet.Type) {
                                if (!(temp_pet.ID in triedPets) && !foundNew) {
                                  ...
                                  let ind = tempGroup.findIndex(
                                    (temp_repl) => temp_repl.ID === temp_pet.ID,
                                  );
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            <Image alt="X (cross to remove)" src={xIcon} fill />
                          </div>
                        </div>
                        {/* Pet Placement */}
                        <div
                        ...
                        >
                          <select
                            ...
                            onChange={(choice) => {
                              ...
                              setPetWhiteList((curr) => {
                                let temp = [...curr];
                                let tempPet = temp.find(
                                  (inner_pet) => inner_pet.id === pet.id,
                                );
                          ...
                          >
                            <option value={PLACEMENT_BLACKLIST}>
                              Blacklist
                            </option>
                        ...
                        <div
                          ...
                          style={{
                            ...
                            position: "relative",
                            opacity:
                              pet.placement === PLACEMENT_BLACKLIST
                                ? "0.4"
                                : "",
                            display: "flex",
                            justifyContent: "center",
                        ...
                        >
                          {pet.placement === PLACEMENT_TEAM && (
                            <div
                              style={{
                                marginLeft: showGreen || showRed ? "22px" : "",
                              }}
                            >
                              <select
                                className="importantText"
                                ...
                                onChange={(choice) => {
                                  setPetWhiteList((curr) => {
                                    ...
                                    tempPet.parameters.team = Number(
                                      choice.target.value,
                                    );
                                    return temp;
                                  });
                              ...
                              >
                                {Array.apply(null, Array(Number(numTeams))).map(
                                  (e, index) => {
                                    return (
                ...
                >
                  ...
                  <div style={{ margin: "0 0px 0 0px" }}>
                    {leftOver1Pets.map((pet, index) => {
                      let staticPetData = petNameArray.find(
                        (staticPetDatum) => staticPetDatum.petId === pet.ID,
                      );
                        ...
                        >
                                  ...
                                  onClick={(e) => {
                                    setPetWhiteList((curr) => {
                                      ...
                                      if (!pet_inner) {
                                        ...
                                        throw new Error(
                                          `should not have an existing pet in this list!`,
                                        );
                                      }
                                      return temp;
                                    });
                          ...
                          >
                            {helper.roundTwoDecimal(
                              petHelper.calculatePetBaseDamage(
                                pet,
                                defaultRank,
                              ),
                            )}
