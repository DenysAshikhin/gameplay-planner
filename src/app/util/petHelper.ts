import general_helper from './helper';
...
export type groupScore = {
    groupScore: number,
    baseGroupScore: number,
    groupScoreMax: number,
    groupRankScore: number,
    dmgCount: number,
    timeCount: number,
    synergyBonus: number,
    cardPowerCount: number,
    expRewardCount: number,
    cardXpCount: number,
    rpRewardCount: number,
    tokenRewardCount: number,
    tokenModif: number,
    tokenMult: number,
};
...
const helper = {
    ...
    calculateGroupScore: function (group, defaultRank): groupScore {
        ...
        let tokenMult = 0;
        let tokenModif = 0;
        const typeCounts: { [index: string]: number } = {};
    ...
    getBestDamagePets: function (petsCollection, defaultRank, other) {
        ...
        //As required + miscellaneous pets are added, keep track of top 4 strongest -> to prevent adding non-special weak pets
        ...
        let finalPetsCollection = Object.values(finalCollection) as any[];
    ...
    calcBestDamageGroup: function (PETSCOLLECTION, defaultRank, numGroups, other) {
            ...
            const f = (start, prevCombination) => {
                ...
                if (prevCombination.length > 0) {
                    ...
                    if (validTeam) {
                        ...
                        //First confirm the combination satisfies all bonuses
                        for (let i = 0; i < bonusList.length; i++) {
                            let bonus = bonusList[i];
                            let pass = false;

                            //Pet being forcefully included, needs to be here
                            if (bonus.placement === 'team') {
                                let currCount = 0;
                                for (let j = 0; j < prevCombination.length; j++) {
        ...
        for (let g = 0; g < numGroups; g++) {
            ...
            for (const [key, value] of Object.entries(whitelist)) {
                let value2 = value as any;

                if (value2.parameters.team === g) {

                    if (!petsMap[value2.pet.ID]) {
                        continue;
                    }

                    whiteListReqPets.push(value);
                    petsCollection.push(value2.pet);
                    requiredPetsOverall.push(value2.pet);
                }
            }
            ...
            for (const [key, value2] of Object.entries(requiredPetBonusMap)) {
            ...
            //Only try rel, if it's possible (meaning don't skip custom bonuses)
                ...
                if (activeBonuses.length > 0 || individualRel.length > 0) {
                    ...
                    for (let j = 0; j < individualRel.length; j++) {
                        let curBonus = individualRel[j] as any;
                    ...
                    if (added) {
                        ...
                        if (combinations_rel === -1 && whiteListReqPets.length === 0) {
                            ...
                            for (let x = 0; x < bonusList.length; x++) {
                                const bonusListElement = bonusList[x] as any;

                                if (bonusListElement.id in whitelistRel && combinations_rel.team.find((inner_pet) => inner_pet.ID === bonusListElement.id)) {
                                    delete whitelistRel[bonusListElement.id]
                                }
                            }
                        }
                    }
    ...
    calcBestTokenGroup: function (petsCollection, defaultRank, numGroups, other) {
        ...
        for (let g = 0; g < numGroups; g++) {
            ...
            //Only 1 token pet left,
                ...
                let cutOff = percent * (avgdMaxDmg as groupScore).groupScore; //% of highest available pet's base damage
    ...
    findBestTeam: function (data, parameters) {
        ...
        for (const [key, value2] of Object.entries(petWhiteList)) {
            const value = value2 as any;
        ...
        res = this.populatePets(data, {
            manualEnabledPets: parameters.manualEnabledPets,
            ground: groundPets.length < groundLimit,
            air: airPets.length < airLimit,
            banned: bannedPets
        });
        petList = res.petList;
        ...
        for (const [key, value] of Object.entries(currentBonuses)) {
            numBonuses++;
            (value as any).label = BonusMap[key].label;
        }
