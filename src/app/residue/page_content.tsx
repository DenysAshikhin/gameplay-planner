...
const ResidueCard = ({
  ...
  return (
    <div
      className="importantText residue-card"
      onMouseEnter={() => {
    ...
    >
      {sweet_locked && (
          ...
          >
            <div className="residue-card__header"></div>
            <div className="residue-card__body">
              <Image
                src={residueMap["locked"].sweetlocked}
                fill
                unoptimized
                priority
                alt="sweet potatoe lock"
              />
          </div>
        </div>
      )}
      {skull_locked && (
        ...
        >
          ...
          <div className="residue-card__body">
            <Image
              src={residueMap["locked"].skulllocked}
              fill
              unoptimized
              priority
              alt="sweet potatoe lock"
      ...
      {!key_locked && (
        <>
          <div className="residue-card__header">
            <div>{locked ? `?????` : `${params.label}: ${level}`}</div>
            {((finishedBuying && needPurchase) || futurePurchase) && !locked && (
              <div className="future-purchase" style={futurePurchase ? { color: "yellow" } : {}}>
                <div>{futurePurchase ? `${level + 1}` : `${desiredLevels + level}`}</div>
                <div>{futurePurchase ? `+1` : `+${desiredLevels}`}</div>
              </div>
            )}
          </div>
          <div className="residue-card__body">
            <div style={{ position: "absolute", right: "9px", top: "9px", zIndex: "2" }}>
              <MouseOverPopover
                tooltip={
                  <div>
                    <div>{`Cost: ${params.cost(level).toExponential(2)}`}</div>
                    <div>
                ...
                {weight !== params.weight(asc_level) && (
                  ...
                  >
                    <Image
                      ...
                      unoptimized
                      priority
                      alt="reset, 2 arrows in a circle"
                    />
                  </div>
                )}
              </div>
            ...
            {!!locked && (
              <Image
                ...
                src={residueMap["locked"].img}
                unoptimized
                priority
                alt={`locked bonus image from in game`}
              />
            )}
            {!locked && (
              <Image
                ...
                unoptimized
                priority
                alt={`${params.key} bonus from in game`}
              />
            )}
            {!!needPurchase && finishedBuying && !locked && (
              <Image
                ...
                unoptimized
                priority
                alt={`Green border to indicate an upgrade should be purchased`}
              />
            )}
            {(!finishedBuying || futurePurchase) && (
              <Image
                ...
                unoptimized
                priority
                alt={`Yellow border to indicate an upgrade is still autobuying`}
              />
            )}
          </div>
        </>
      ...
      <div className="suggestion-card__body">
        ...
        >
          ...
          >
            ...
            <div>{`${data.desiredLevel}`}</div>
            <div style={{ marginLeft: "auto" }}>{`+${data.desiredLevel - data.start}`}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    while (keepLooping) {
      ...
      for (const [key, value] of Object.entries(parentWeights)) {
        ...
        temp_values.push({
          key: key,
          baseCost: baseCost,
          level: data[temp_obj.key] + purchases[key].levels,
        });
      ...
      if (bestValue && available) {
        if (currentResidue.lessThan(bestValue.baseCost)) {
          ...
          purchases[bestValue.key].runningCost = mathHelper.addDecimal(
            purchases[bestValue.key].runningCost,
            bestValue.baseCost,
          );
      ...
      >
        ...
        <div className="wrapper residue">
          <div className="wrapper__title importantText">
            Current Residue&nbsp;
            {stillBuying && <span className="wrapper__title--addon">- AUTO STILL BUYING</span>}
          </div>

          {/* Card List */}
          <div className="card-list">
            {Object.entries(residueMap)
              .filter((value) => value[0] !== "locked")
              ...
              .map((value, index) => {
                ...
                return (
                  <ResidueCard
                    ...
                    futurePurchase={
                      !!finalObject[key]?.futurePurchase ? finalObject[key]?.futurePurchase : false
                    }
        ...
        {!data.ASCResidueTakeAway && (
          <div className="wrapper suggested-order">
            <div className="wrapper__title importantText">Suggested Purchase Order</div>
            ...
            <div className="card-list importantText">
              ...
              {!Object.entries(finalObject).some(([key]) => finalObject[key]?.levels > 0) && (
                <div>Nothing to purchase yet</div>
              )}
              {Object.entries(residueMap)
                .filter((value) => value[0] !== "locked")
                .sort((a, b) => a[1].order - b[1].order)
