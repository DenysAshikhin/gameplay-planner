...
export default function Contagion() {
    ...
    const [contagionWeights, setContagionWeights] = useState({} as { key: string, value: number });
    ...
    const final_gh_amounts = useMemo(() => {
        ...
        for (const [key, value] of Object.entries(contagionWeights)) {
            totalWeights += value as number;
            return_obj[key] = { weight: value };
        }
        for (const [key, value] of Object.entries(contagionWeights)) {
            let gh_amount = Math.floor((value as number) / totalWeights * gh_available);
                                    ...
                                    >
                                        <div style={{ width: "49%", height: '100%' }}>
                                            <ContagionLine
                                                ...
                                                setContagionWeights={setContagionWeights} key={index} data={data} contagion={data.GrasshopperCollection[index]} />
                                        </div>
                                        <div style={{ width: "49%", height: '100%' }}>
                                            {!!data.GrasshopperCollection[index + 1] && (
                                                <ContagionLine
                                                    ...
                                                    setContagionWeights={setContagionWeights} key={index} data={data} contagion={data.GrasshopperCollection[index + 1]} />
                                            )}
                                        </div>
                                    </div>
                                )
