...
function shouldIgnore(file, serverIgnoreFn, reasons, cachedIgnoreFiles) {
    ...
    if ([
        ...reason.parents.values()
    ].every((parent)=>shouldIgnore(parent, serverIgnoreFn, reasons, cachedIgnoreFiles))) {
        cachedIgnoreFiles.set(file, true);
        return true;
    }
    cachedIgnoreFiles.set(file, false);
        ...
        if (config.experimental.turbotrace) {
            ...
            for (const [set, files] of [
                ...
                for (const file of files){
                    if (!makeIgnoreFn(set === minimalServerTracedFiles ? minimalServerIgnores : serverIgnores)(_path.default.join(traceContext, file))) {
            ...
            for (const [entries, tracedFiles] of [
                ...
                for (const file of entries){
                    ...
                    for (const curFile of curFiles || []){
                        ...
                        if (!shouldIgnore(curFile, makeIgnoreFn(tracedFiles === minimalServerTracedFiles ? minimalServerIgnores : serverIgnores), reasons, tracedFiles === minimalServerTracedFiles ? cachedLookupIgnoreMinimal : cachedLookupIgnore)) {
            ...
            await Promise.all([
                ...
                for (const file of [
                    ...
                    const curFiles = parentFilesMap.get(_path.default.relative(outputFileTracingRoot, file));
                    for (const curFile of curFiles || []){
                        if (!shouldIgnore(curFile, makeIgnoreFn(routesIgnores), reasons, cachedLookupIgnoreRoutes)) {
                            const filePath = _path.default.join(outputFileTracingRoot, curFile);
